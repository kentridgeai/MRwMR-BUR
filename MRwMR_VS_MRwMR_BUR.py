...
MLP = MLPClassifier(alpha=0, hidden_layer_sizes=(20,15,7,3),max_iter=1000,learning_rate_init=0.01)
...
def Calculation(order,train_data,train_label,vali_data,vali_label):
	...
	for index in range(0,num_of_features):
		...
		pred_result = KNN.predict(feature_vali)
		KNN_acc.append(metrics.accuracy_score(vali_label,pred_result))

	return MLP_acc,SVM_acc,RF_acc, KNN_acc
...
for i in range(0,20):
	...
	for iter in range(0,10):
		...
		MLP_result,SVM_result,RF_result, KNN_result = Calculation(order_gen,x_train,y_train,x_test,y_test)
